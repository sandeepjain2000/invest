...
rem --- Edit these overrides (leave blank to use ca_bulk_config.json defaults) ---
set "GOAL="
set "BATCH="
set "HEADED="

rem Defaults in ca_bulk_config.json: goal_emails_per_run=1000, profiles_per_batch=50

if not exist "ca_bulk_config.json" (
  if exist "ca_bulk_config.example.json" (
    echo Creating ca_bulk_config.json from example...
    copy /Y ca_bulk_config.example.json ca_bulk_config.json >nul
  ) else (
    echo ERROR: ca_bulk_config.example.json not found.
    pause
    exit /b 1
  )
)

if not exist "ca_connect_credentials.json" (
  echo ERROR: ca_connect_credentials.json not found.
  echo Copy ca_connect_credentials.example.json and add your CA Connect login.
  pause
  exit /b 1
)
...
echo   Config: ca_bulk_config.json
echo ============================================================
echo.
pause
