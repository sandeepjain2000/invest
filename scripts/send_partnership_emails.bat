...
echo   Folder: %CD%
echo.
echo   Step 1: Google scrape only (no CA Connect — use scripts\run_ca_bulk_import.bat for CAs)
echo   Step 2: Send emails — 8 CA slots from ca_bulk.db + other industries
echo.
echo   Settings: config\sender_config.json  ^|  browser=%BROWSER%  region=%REGION%
echo ============================================================
echo.
pause
