...
rem HARVEST = pull new CA emails from ICAI (browser). Not the same as ca_bulk_status.bat.
...
echo ============================================================
echo   HARVEST CA emails from ICAI  (caconnect.icai.org)
echo   Folder: %CD%
echo   Database: data\db\ca_bulk.db
echo.
echo   Check progress only:  scripts\ca_bulk_status.bat
echo   Send emails later:    scripts\send_ca_emails.bat
echo.
echo   Browser will open - do not close this window during harvest.
echo ============================================================
echo.
echo Press any key to START the harvest...
pause >nul
echo.
...
echo.
echo [Run] Harvesting CA profiles from ICAI (resume from last checkpoint)...
set "RUN_ARGS=run"
